...
let { answers } = require("./codes_await");
let cTab;
...
(async function fn(){
    try{
        ...
        await  cTab.type("input[name='username']" , "wecaboy740@bsmitao.com" , { delay: 2 });
        await cTab.type("input[name='password']", "Shibbujain2001", { delay: 2 });
...
async function questionSolver(qName, code, mainPageLink) {
    ...
    // next page element wait that is not pesent on the current page 
    // Because checkbox is also present current page
